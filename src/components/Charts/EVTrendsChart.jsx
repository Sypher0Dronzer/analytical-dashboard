import { useMemo } from "react";
import { BarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, CartesianGrid } from "recharts";
import { useStore } from "../../store/fetchAll";
import { motion } from "framer-motion";

// Define a consistent color palette
const colors = [
  "#6366F1",
  "#8B5CF6",
  "#EC4899",
  "#10B981",
  "#F59E0B",
  "#EF4444",
  "#3B82F6",
  "#22C55E",
  "#EAB308",
  "#DB2777",
  "#14B8A6",
  "#9333EA",
  "#F87171",
  "#FACC15",
  "#4ADE80",
  "#60A5FA",
];

// Custom Tooltip for better styling
const CustomTooltip = ({ active, payload }) => {
  if (!active || !payload || payload.length === 0) return null;

  return (
    <div className="bg-gray-900 p-3 border border-gray-700 rounded-lg shadow-lg text-gray-200">
      <p className="text-sm font-semibold mb-2">{payload[0].payload.city}</p>
      <div className="grid grid-cols-2 gap-2 text-xs">
        {payload.map((entry, index) => (
          <div key={index} className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full" style={{ backgroundColor: entry.color }} />
            <span>{entry.name}:</span>
            <span className="font-semibold">{entry.value}</span>
          </div>
        ))}
      </div>
    </div>
  );
};

const EVTrendsChart = () => {
  const { cityEvAdoption } = useStore();

  // Prepare chart data
  const chartData = useMemo(() => {
    if (!cityEvAdoption.length) return [];

    // Extract all unique years from data
    const allYears = [...new Set(cityEvAdoption.flatMap(city => city.yearlyData.map(y => y.year)))].sort();

    return cityEvAdoption.map(({ city, yearlyData }) => {
      const yearMap = Object.fromEntries(yearlyData.map(({ year, count }) => [year, count]));
      return { city, ...Object.fromEntries(allYears.map(year => [year, yearMap[year] || 0])) };
    });
  }, [cityEvAdoption]);

  return (
    <motion.div
      className="p-6 bg-gray-800 bg-opacity-50 backdrop-blur-md shadow-lg rounded-xl border border-gray-700"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.2 }}
    >
      <h2 className="text-lg font-medium mb-4 text-gray-100">
        EV Registration Trends in Top 10 Cities
      </h2>
      
      <ResponsiveContainer width="100%" height={450}>
        <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 50 }}>
          <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} />
          <XAxis dataKey="city" angle={-30} textAnchor="end" height={60} tick={{ fontSize: 13, fill: "#9ca3af" }} />
          <YAxis tick={{ fontSize: 13, fill: "#9ca3af" }} />
          <Tooltip content={<CustomTooltip />} />
          <Legend wrapperStyle={{ fontSize: "12px", fontWeight: "thin" }} />

          {Object.keys(chartData[0] || {}).filter(key => key !== "city").map((year, index) => (
            <Bar
              key={year}
              dataKey={year}
              stackId="a"
              stroke={colors[index % colors.length]}
              fill={colors[index % colors.length]}
              fillOpacity={0.8}
            />
          ))}
        </BarChart>
      </ResponsiveContainer>
     <p className=" mb-4 text-gray-400 text-center">This chart showcases EV registration trends in the top 10 cities, revealing adoption patterns over the years.</p>
    </motion.div>
  );
};

export default EVTrendsChart;
